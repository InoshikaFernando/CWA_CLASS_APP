# progress

Cross-subject progress reporting and dashboards. Aggregates data from the subject apps (`maths`, `coding`, …) into per-student and per-class views. Owns the JSON API used by client-side charts on the dashboards.

Mostly a **utility / view-only** app: the dashboards own no data, they read it from the subject apps. The one exception is `PeriodReport` (CPP-388), which has to be stored because an end-of-period report is a frozen snapshot rather than a live view.

## Key models

`ProgressReportSetting` — which periods a school, department or class actually
sends. Reports are **off until switched on** and **manual until set to automatic**;
the cascade is class → department → school → off, and each flag inherits
independently (`NULL` = inherit). The same rows carry the schedule an automatic
run fires on.
Configured at `/progress/reports/settings/` by a Head of Institute.

`PeriodReport` — one student's weekly / monthly / term progress report. `data` holds the whole computed snapshot (totals, per-topic accuracy, per-homework attempts, trend, awards); `notified_at` / `parent_emailed_at` track delivery and are what make the nightly generator idempotent. See [`docs/specs/CPP-388_period_progress_reports.md`](../docs/specs/CPP-388_period_progress_reports.md).

Everything else is read from:
- `homework.HomeworkSubmission`, `homework.HomeworkStudentAnswer`
- `maths.StudentFinalAnswer` — topic/mixed quizzes and times tables
- `maths.BasicFactsResult` — basic facts
- `worksheets.WorksheetSubmission`
- `maths.BasicFactsResult`, `maths.StudentAnswer`, `maths.TimeLog`, `maths.TopicLevelStatistics`
- `coding.StudentProblemSubmission`, `coding.CodingTimeLog`

## Period reports (CPP-388)

| Module | Responsibility |
|--------|----------------|
| `periods.py` | Which window a period covers, and which windows closed on a given day |
| `report_settings.py` | The school → department → class opt-in cascade |
| `visibility.py` | Whether a student/parent sees the reports nav link at all |
| `views_settings.py` | The Head of Institute configuration page |
| `views_preview.py` | Staff preview — the list, one student's live report + PDF, and manual send |
| `reports.py` | Builds the snapshot — totals, topics, attempts, trend, awards |
| `services.py` | Generates reports and delivers them (in-app notification; term email) |
| `outreach.py` | Whole-school coverage (CPP-422) — who has nothing to show, why, and the note their parents get |
| `pdf.py` | Renders a report to PDF with ReportLab, charts included |
| `access.py` | Who may read a report (student / parent / class teacher / HoI) |
| `views_reports.py` | List, detail (Chart.js) and PDF-download views |

Generated by cron:

```bash
python manage.py generate_progress_reports        # whatever closed today
```

### Reviewing a term that is still running (CPP-425)

The preview's term view carries a selector over every term that has begun. The
default is the term that just finished — what it always showed — and the one
still running can be chosen beside it, reported from its start **to today** and
labelled `(to date)`.

A running term is **review only**. A term report keys on the term's start date,
so a row stored mid-term is the row the real end-of-term report needs:
`generate_report` raises `PartialWindowError` rather than write one, the send
POST refuses it, and the page shows no button to press.

### Whole-school coverage (CPP-422)

By default a run reaches only students in a class with that period switched on,
and says nothing at all for a period with no activity. A school that switches
*Cover every student in the school* on instead has every active `SchoolStudent`
covered, and the ones with nothing to show get one email to their parents giving
the reason — **no active subscription** (with the sign-in link and the school's
`subscription_discount_code`, when it is still valid) or **no work done**.

Off until switched on, read from the school setting row only, and stamped one
row per student per period in `PeriodReportNotice` so a re-run re-mails nobody.
Skipped for a single-class run and for a `?subscribed=1` run, which the preview
page mirrors so the page and the send button cannot disagree. Full spec:
[`docs/SPEC_REPORT_AUTOMATION_WHOLE_SCHOOL.md`](../../docs/SPEC_REPORT_AUTOMATION_WHOLE_SCHOOL.md).

## URL prefix & key routes

Mounted at the project root.

- `/student-dashboard/` — student's overall progress home
- `/student/<id>/progress/` — teacher view of a student's progress

Period reports live under the `progress:` namespace (`progress/report_urls.py`,
kept separate so the two routes above keep their un-namespaced names):

- `/progress/reports/` — `progress:period_report_list`
- `/progress/reports/<id>/` — `progress:period_report_detail`
- `/progress/reports/<id>/pdf/` — `progress:period_report_pdf`
- `/progress/reports/settings/` — `progress:report_settings` (HoI / owner / admin)
- `/progress/reports/preview/` — `progress:report_preview` (staff; preview + manual send).
  `?subscribed=1` narrows the scope to students whose own subscription is active
  or trialing (`billing.selectors`); the filter is applied to the report plan, so
  "Generate and send" covers exactly the rows on screen.
- `/progress/reports/preview/report/` — `progress:report_preview_detail` (staff; one student's report, computed and discarded)
- `/progress/reports/preview/report/pdf/` — `progress:report_preview_pdf` (staff; the same, as the parent's PDF)

## API

JSON endpoints for charts. Mounted at both `/api/` (legacy) and `/api/v1/` (canonical):

```python
path('api/v1/', include('progress.api_urls')),
path('api/',    include('progress.api_urls')),  # legacy, kept for compatibility
```

## Integration

In `settings.py`:

```python
INSTALLED_APPS = [..., 'progress', ...]
```

In root `urls.py`:

```python
path('', include('progress.urls')),
path('', include('progress.report_urls', namespace='progress')),
path('api/v1/', include('progress.api_urls')),
path('api/',    include('progress.api_urls')),  # legacy
```

## Dependencies

- **homework** — submissions and per-question answers; the period report's data source.
- **worksheets** — worksheet completions in the reported window.
- **maths** — quiz attempts, time logs, topic stats; `Question.topic` for the report's topic breakdown.
- **coding** — problem submissions and time logs.
- **classroom** — `Subject`, `Level`, `Topic`, `ClassRoom`, `Term`, `School`, `ParentStudent`, `Notification`, `email_service`.
- **accounts** — `CustomUser` for student/teacher identity.
- **reportlab** — server-side PDF rendering (pure Python; no system libraries).

## External services

SMTP, indirectly: the end-of-term parent email goes out through
`classroom.email_service.send_templated_email`.
