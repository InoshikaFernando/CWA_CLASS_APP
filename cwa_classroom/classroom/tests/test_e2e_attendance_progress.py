...
# ---------------------------------------------------------------------------
# 8. StudentProgressReportFilterTest
# ---------------------------------------------------------------------------

class StudentProgressReportFilterTest(_BaseAttendanceProgressTest):
    """The report overview's Subject dropdown is scoped to the chosen department."""

    @classmethod
    def setUpTestData(cls):
        super().setUpTestData()
        from classroom.models import DepartmentSubject
        # A second department (IT) whose subject (Coding) has no class yet.
        ...
        # A class in IT so the department is selectable in the filter.
        _create_classroom(cls.school, cls.it_dept, cls.coding, 'Coding 101')
    ...
    def test_subjects_scoped_to_selected_department(self):
        self._login()
        url = reverse('student_progress_report')
        # IT department → its subject (Coding), not Maths (the other department).
        resp = self.client.get(f'{url}?department={self.it_dept.id}')
        names = [s.name for s in resp.context['subjects']]
        self.assertIn('Coding', names)
        self.assertNotIn('Mathematics', names)

    def test_subjects_unscoped_when_no_department(self):
        self._login()
        resp = self.client.get(reverse('student_progress_report'))
        names = [s.name for s in resp.context['subjects']]
        # No department filter → subjects across all accessible classes.
