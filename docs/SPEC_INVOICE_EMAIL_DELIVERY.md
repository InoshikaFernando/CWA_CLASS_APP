...
- **Admin user lists** (`school_teachers.html`, `partials/students_table.html`) upgrade the existing "Welcome sent / not sent" chip to show the tracked delivery state via the shared `partials/_welcome_email_badge.html`, falling back to the `welcome_email_sent` flag when no log exists. Views annotate each page row with `welcome_email_state` (`_annotate_welcome_email_state` in `views_admin.py`).
